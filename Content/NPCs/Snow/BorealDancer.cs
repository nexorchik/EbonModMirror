...
public class BorealDancer : ModNPC
{
    ...
    public override void AI()
    {
        ...
        if (Helper.TileRaycast.CastLength(NPC.Center - Vector2.UnitY * 6, VectorDistance, VectorDistance.Length(), false) >= VectorDistance.Length() - 0.4f)
            NPC.ai[2] = 180;
        NPC.ai[2]--;
        ...
        if (NPC.ai[2] > 0)
        {
            ...
            if ((int)NPC.ai[0] == 1)
            {
                ...
                {
                    MPUtils.NewProjectile(NPC.GetSource_FromThis(), Helper.TileRaycast.Cast(NPC.Center - new Vector2(-NPC.direction * 15, 35), Vector2.UnitY, 500, true) + new Vector2(0, 3), Vector2.Zero, ProjectileType<BorealSpike>(), NPC.damage, 0, ai0: 2, ai1: NPC.direction);
        ...
        if (Helper.TileRaycast.CastLength(NPC.Center - Vector2.UnitY * 12, new Vector2(NPC.velocity.X, 0), 45, false) < 12 && XVelocityModule > 0.4f)
        {
            for (int u = 0; u < 15; u++)
            {

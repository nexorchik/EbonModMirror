...
public class Ram : ModNPC
{
    ...
    {
        return spawnInfo.Player.ZoneForest && !Main.bloodMoon ? 0.02f : 0;
    }
    ...
    public override void AI()
    {
        ...
        if (!spawn)
        {
            ...
            if (Main.rand.NextBool(4) && NPC.Center.Distance(Main.LocalPlayer.Center) < 600)
                SoundEngine.PlaySound(Sounds.sheep.WithPitchOffset(-0.3f), NPC.Center);
            NPC.netUpdate = true;
            spawn = true;
        }
        lastClicked--;
        if (Main.rand.NextBool(2000) && NPC.Center.Distance(Main.LocalPlayer.Center) < 600)
            SoundEngine.PlaySound(Sounds.sheep.WithVolumeScale(0.35f).WithPitchOffset(-0.3f), NPC.Center);
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        if (sheared)
            tex = Assets.NPCs.Overworld.Critters.Ram_Naked.Value;
    ...
    public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D tex = Assets.NPCs.Overworld.Critters.Ram_Wool.Value;
        if (dyeId > 0 && !sheared)
        {
            string name = Main.LocalPlayer.name;
            name.ApplyCase(LetterCasing.LowerCase);

		...
		switch ((States)AIState)
		{
			...
			case States.Leap:
			{
				...
				if (NPC.Grounded() && AITimer > 3)
				{
					Projectile.NewProjectileDirect(NPC.GetSource_FromAI(),
						Helper.Raycast(NPC.Center, Vector2.UnitY, 100) + new Vector2(NPC.direction * 18, 0), Vector2.Zero,
			...
			case States.ChargeExplosion:
			{
				...
				if (AITimer > 50)
				{
					...
					dustPosition = Helper.Raycast(dustPosition - new Vector2(0, 40), Vector2.UnitY, 100);

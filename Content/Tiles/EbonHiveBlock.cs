...
public class EbonHiveBlockSpecial : ModTile
{
    ...
    public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
    {
        ...
        while (distance > 6 && !float.IsNaN(distance))
        {
            ...
            //Draw chain
            spriteBatch.Draw(Helper.GetTexture(Helper.AssetPath+"Items/Accessories/HeartChain").Value, center + zero,
                null, Lighting.GetColor(new(i, j)), projRotation,
                new Vector2(12 * 0.5f, 6 * 0.5f), 1f, SpriteEffects.None, 0);
        }
        spriteBatch.Draw(Request<Texture2D>(Helper.AssetPath+"Items/Accessories/EbonianHeartNPC").Value, finalCenter + zero,
                    frame, Lighting.GetColor(new(i, j)), 0,
                    new Vector2(34 * 0.5f, 34 * 0.5f), 1f, SpriteEffects.None, 0);
        spriteBatch.Draw(Helper.GetTexture(Helper.AssetPath+"Items/Accessories/EbonianHeartNPC_Glow").Value, finalCenter + zero,
                    frame, Color.White, 0,
                    new Vector2(34 * 0.5f, 34 * 0.5f), 1f, SpriteEffects.None, 0);
        ...
        frame = new Rectangle(0, frame.Y, Request<Texture2D>(Helper.AssetPath+"Items/Accessories/EbonianHeartNPC").Value.Width, Request<Texture2D>(Helper.AssetPath+"Items/Accessories/EbonianHeartNPC").Value.Height / 3);
        return true;
    }

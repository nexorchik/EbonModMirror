    ...
    public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
    {
        for (int k = -1; k < 2; k++)
        {
            ...
            while (distance > 4 && !float.IsNaN(distance))
            {
                ...
                //Draw chain
                spriteBatch.Draw(Helper.GetTexture(Helper.AssetPath+"Items/Accessories/BrainAcc_Chain").Value, center + zero,
                    null, Lighting.GetColor(new(i, j)), projRotation,
                    new Vector2(8 * 0.5f, 4 * 0.5f), 1f, SpriteEffects.None, 0);
            }
            spriteBatch.Draw(Request<Texture2D>(Helper.AssetPath+"Items/Accessories/TinyBrain").Value, finalCenter + zero,
        ...
        frame = new Rectangle(0, frame.Y, Request<Texture2D>(Helper.AssetPath+"Items/Accessories/TinyBrain").Value.Width, Request<Texture2D>(Helper.AssetPath+"Items/Accessories/TinyBrain").Value.Height / 4);
        return true;
    }

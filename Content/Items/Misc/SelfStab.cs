...
{
    public override string Texture => Helper.AssetPath + "Items/Misc/SelfStab";
...
public class SelfStabP : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Projectile.timeLeft == 22)
        {
            ...
            player.Hurt(info);
            player.immuneTime = 0;
            SoundEngine.PlaySound(Sounds.fleshHit, player.Center);

            Helper.AddCameraModifier(new PunchCameraModifier(Projectile.Center, Main.rand.NextVector2Unit(), 5, 6, 30, 1000));
            player.GetModPlayer<WeaponPlayer>().stabDirection = new Vector2(-player.direction, Main.rand.NextFloat(-1, 0));
            for (int i = 0; i < 5; i++)
            {
...
{
    public override string Texture => Helper.AssetPath+"Buffs/SelfStabB";
    ...
    public override void Update(Player player, ref int buffIndex)
    {
        ...
        Vector2 dir = player.GetModPlayer<WeaponPlayer>().stabDirection.RotatedByRandom(MathHelper.PiOver4 / 2) * (Main.rand.NextBool(5) ? -1 : 1);

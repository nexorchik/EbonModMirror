...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Ranged/Spitvine";
    ...
    public override void AI()
    {
        ...
        if (--Projectile.ai[1] <= 0)
            foreach (NPC npc in Main.npc)
            {
                ...
                {
                    if (npc.Center.X.InRange(Projectile.Center.X, npc.width + 40) && npc.Center.Y.InRange(Projectile.Center.Y, npc.height + 40))
                    {
                        float f = Main.rand.NextFloat(1, 5) * Main.rand.NextFloatDirection();

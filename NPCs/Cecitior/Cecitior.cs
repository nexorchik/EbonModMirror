        ...
        else if (AIState == Phase2ClawBodySlam)
        {
            ...
            else
            {
                ...
                if (Helper.TRay.CastLength(NPC.Center, -Vector2.UnitY, NPC.height * 2, true) < NPC.height && AITimer2 == 0 && AITimer > 45)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        Projectile.NewProjectileDirect(NPC.GetSource_FromThis(), NPC.Center - new Vector2(0, 100), Main.rand.NextVector2Circular(7, 7), ProjectileType<HostileGibs>(), 40, 0, 0, 0);

...
public class ArchmageCutsceneMartian : ModNPC
{
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D burnt = ExtraSpriteTextures.ArchmageX_Burnt.Value;
        ...
        Texture2D fireG = ExtraSpriteTextures.XFire_Gray.Value;
        Texture2D fireP = ExtraSpriteTextures.XFire_Purple.Value;
        ...
        if (beamAlpha > 0)
        {
            ...
            if (vertices.Count > 2)
            {
                Helper.DrawTexturedPrimitives(vertices.ToArray(), PrimitiveType.TriangleStrip, ExtraTextures.saucerBeam.Value, false);
            }
            vertices.Clear();
            for (float i = 0; i < 1; i += 0.001f)
            {
            ...
            if (vertices.Count > 2)
            {
                Helper.DrawTexturedPrimitives(vertices.ToArray(), PrimitiveType.TriangleStrip, ExtraTextures.Tentacle.Value, false);
        ...
        if (!NPC.AnyNPCs(NPCID.MartianSaucer))
        {
            if (AITimer == 0)
            {
                ...
                if (!Main.dedServ)
                {
                    ...
                    Helper.AddCameraModifier(modifier);
                }
                ...
                for (int i = 0; i < 20; i++)
                    MPUtils.NewProjectile(null, NPC.Center, Main.rand.NextVector2Unit() * Main.rand.NextFloat(10, 20), ProjectileType<XCloudVFXExtra>(), 0, 0);
                for (int i = 0; i < 15; i++)
            ...
            if (AITimer < 100)
            {
                rightArmRot = Utils.AngleLerp(rightArmRot, -NPC.direction * 0.25f + MathHelper.ToRadians(-5f - AITimer * 0.1f) * NPC.direction, 0.3f);
                leftArmRot = Utils.AngleLerp(leftArmRot, NPC.direction * 0.25f + MathHelper.ToRadians(5f + AITimer * 0.1f) * NPC.direction, 0.3f);
            }
            if (AITimer > 100 && AITimer < 180)
            {
                rightArmRot = Utils.AngleLerp(rightArmRot, -NPC.direction * 0.15f + MathHelper.ToRadians(-5f) * NPC.direction, 0.3f);
                leftArmRot = Utils.AngleLerp(leftArmRot, NPC.direction * 0.15f + MathHelper.ToRadians(5f) * NPC.direction, 0.3f);
            }
            if (AITimer > 180 && AITimer < 265)
            {
                float fac = MathHelper.Lerp(0.2f, 1, (AITimer - 180) / 85f);
                rightArmRot = Utils.AngleLerp(rightArmRot, -NPC.direction * 0.5f + (MathHelper.ToRadians(-5f) - MathF.Sin(AITimer * 0.35f * fac) * .6f) * NPC.direction, 0.2f);
                leftArmRot = Utils.AngleLerp(leftArmRot, NPC.direction * 0.5f + (MathHelper.ToRadians(5f) + MathF.Sin(AITimer * 0.35f * fac) * .6f) * NPC.direction, 0.2f);
            }
            if (AITimer >= 265)
            {
                rightArmRot = Utils.AngleLerp(rightArmRot, -NPC.direction * 0.65f + MathHelper.ToRadians(-5f) * NPC.direction, 0.2f);
                leftArmRot = Utils.AngleLerp(leftArmRot, NPC.direction * 0.65f + MathHelper.ToRadians(5f) * NPC.direction, 0.2f);
            }
            // DIALOGUE
            if (AITimer == 1)
            {
            ...
            if (AITimer == 1900)
            {
                ...
                MPUtils.NewProjectile(null, NPC.Center, Main.rand.NextVector2CircularEdge(7, 7), ProjectileID.CrossGraveMarker, 0, 0, -1);
                NPC.checkDead();
            }
            // VFX
            ...
            if (AITimer == 520)
            {
                ...
                Helper.AddCameraModifier(new PunchCameraModifier(NPC.Center, Main.rand.NextVector2Unit(), 15, 6, 30, 1000));
            }
            if (AITimer > 600)

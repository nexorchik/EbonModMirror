...
public class VileTear : ModNPC
{
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D tex2 = Images.ExtraSprites.Corruption.Textures.VileTear_Glow.Value;
        ...
        switch (AIState)
        {
            case 0:
                ...
                if (AITimer > 200)
                    NPC.velocity *= 0.9f;
                if (AITimer > 1)
                    NPC.rotation = Utils.AngleLerp(NPC.rotation, NPC.velocity.ToRotation() + MathHelper.Pi, 0.25f);
                if (AITimer >= 230)
                {
                    NPC.netUpdate = true;
            ...
            case 1:
                ...
                if (AITimer < 70)
                {
                    ...
                    NPC.rotation = Utils.AngleLerp(NPC.rotation, Helper.FromAToB(NPC.Center, player.Center).ToRotation() + MathHelper.Pi, 0.25f);
                }
                ...
                if (AITimer == 30)
                {
                    NPC.damage = 40;
                    NPC.velocity *= 0.5f;
                    //MPUtils.NewProjectileServerSide(null, NPC.Center, Helper.FromAToB(NPC.Center, player.Center), ProjectileType<VileTearTelegraph>(), 0, 0);
                ...
                if (AITimer == 70)
                {
                    ...
                    if (AITimer3 == 0)
                    {
                        ...
                        for (int i = -1; i < 2; i++)
                            MPUtils.NewProjectile(null, NPC.Center - new Vector2(32, -40 * i).RotatedBy(NPC.rotation), Helper.FromAToB(NPC.Center + new Vector2(32, 50 * i).RotatedBy(NPC.rotation), p), ProjectileType<RegorgerBolt>(), 20, 0);
                    }
                }
                ...
                if (AITimer > 70 && AITimer < 100 && AITimer3 == 1)
                    MPUtils.NewProjectile(null, NPC.Center + Main.rand.NextVector2Circular(30, 30) + new Vector2(NPC.width / 3, 0).RotatedBy(NPC.rotation).RotatedByRandom(MathHelper.PiOver2), -NPC.velocity.RotatedByRandom(MathHelper.PiOver4) * 0.2f, ProjectileType<TFlameThrower>(), 20, 0);
                if (AITimer > 100)
                {

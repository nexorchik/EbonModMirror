...
public class GarbageRemote : ModItem
{
    ...
    {
        position = Helper.TRay.Cast(player.Center + new Vector2(40 * player.direction, -50), Vector2.UnitY, 1000, true) - new Vector2(0, 23);
    ...
    public override bool? UseItem(Player player)
    {
        if (Main.myPlayer != player.whoAmI)
            Projectile.NewProjectile(null, Helper.TRay.Cast(player.Center + new Vector2(40 * player.direction, -50), Vector2.UnitY, 1000, true) - new Vector2(0, 23), Vector2.Zero, Item.shoot, 0, 0, player.whoAmI);

        SoundEngine.PlaySound(EbonianSounds.garbageSignal.WithVolumeScale(3), player.position);
        return null;
    }
}

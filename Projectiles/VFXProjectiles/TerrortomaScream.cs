...
public class TerrortomaScream : ModProjectile
{
    ...
    {
        Texture2D tex = ExtraTextures.cone4.Value;
        ...
        for (float i = 0; i < max; i++)
        {
            ...
            for (float j = 0; j < 2; j++)
                Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.LawnGreen with { A = 0 } * alpha * 0.5f, angle, new Vector2(0, tex.Height / 2), new Vector2(Projectile.ai[1], alpha) * scale * 2, SpriteEffects.None, 0);
        }
        return false;
    }
